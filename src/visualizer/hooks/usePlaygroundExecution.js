import { paramStr, typeStr } from '@/core/agent';
        ...
        dump.tasks.forEach((task) => {
            if (task.usage) {
                ...
                if (intent && model_name) {
                    modelBriefsSet.add(model_description
 ...
 */
export function usePlaygroundExecution(options) {
    ...
    const handleRun = useCallback(async (value) => {
        ...
        try {
            ...
            if (playgroundSDK.onDumpUpdate) {
                playgroundSDK.onDumpUpdate((_, executionDump) => {
                    ...
                    setInfoList((prev) => {
                        ...
                        const listWithoutCurrentProgress = prev.filter((item) => !(item.type === 'progress' &&
            ...
            if (actionType !== 'aiAct' && deepThink) {
                console.warn('[Playground] Non-aiAct action will be executed without deepThink. deepThink is only forwarded for aiAct.', {
                    actionType,
            ...
            result.result = await playgroundSDK.executeAction(actionType, value, executionOptions);
            ...
            if (typeof result.result === 'object' && result.result !== null) {
                ...
                if (resultObj.reportHTML)
                    result.reportHTML = resultObj.reportHTML;
                if (resultObj.error)
                    result.error = formatError(resultObj.error);
            ...
            if (typeof e === 'object' && e !== null) {
                if (e.dump)
                    result.dump = e.dump;
                if (e.reportHTML)
                    result.reportHTML = e.reportHTML;
            }
        }
        if (interruptedFlagRef.current[thisRunningId]) {
            return;
    ...
    const handleStop = useCallback(async () => {
        ...
        if (thisRunningId && playgroundSDK && playgroundSDK.cancelExecution) {
            try {
                ...
                const cancelResult = await playgroundSDK.cancelExecution(thisRunningId.toString());
