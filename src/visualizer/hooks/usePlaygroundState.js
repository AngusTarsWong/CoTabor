...
import { createStorageProvider, detectBestStorageType, } from '../component/universal-playground/providers/storage-provider';
import { getWelcomeMessageTemplate } from '../utils/constants';
/**
 * Hook for managing playground state
 */
export function usePlaygroundState(playgroundSDK, storage, contextProvider, targetName) {
    // Core state
        ...
        const initializeMessages = async () => {
            ...
            if (storage?.loadMessages) {
                try {
                    ...
                    const hasWelcomeMessage = storedMessages.some((msg) => msg.id === 'welcome');
        ...
        if (storage?.saveMessages && infoList.length > 1) {
            ...
            storage.saveMessages(infoList).catch((error) => {
                ...
                if (error instanceof DOMException &&
                    ...
                    console.warn('Storage quota exceeded - some messages may not be saved persistently');
                }
                else {
    ...
    useEffect(() => {
        if (!contextProvider?.getUIContext || uiContextPreview)
            return;
        contextProvider
            .getUIContext()
    ...
    useEffect(() => {
        const loadActionSpace = async () => {
            ...
            try {
                ...
                const context = uiContextPreview || (await contextProvider?.getUIContext?.());
