...
function checkPointerMoved(prev, cur) {
    return (Math.abs(prev.pointerLeft - cur.pointerLeft) > 1 ||
...
function handleInsight(acc, sf, frame, baseW, baseH) {
    updateImage(acc, sf, baseW, baseH);
    const alreadyAdded = acc.insights.some((ai) => ai.highlightElement === sf.highlightElement &&
...
// ── Main derivation ──
export function deriveFrameState(scriptFrames, frame, baseW, baseH, fps) {
    ...
    if (!acc.img) {
        const firstImgScript = scriptFrames.find((sf) => sf.type === 'img' && sf.img);
        if (firstImgScript) {
