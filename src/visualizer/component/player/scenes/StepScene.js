...
// ── Main Component ──
export const StepsTimeline = ({ frameMap, autoZoom, frame, width: compWidth, height: compHeight, fps, }) => {
    const { scriptFrames, imageWidth: baseImgW, imageHeight: baseImgH, } = frameMap;
    const state = useMemo(() => deriveFrameState(scriptFrames, frame, baseImgW, baseImgH, fps), [scriptFrames, frame, baseImgW, baseImgH, fps]);
    if (!state.img)
        return null;
    ...
    const showCursor = camera.pointerLeft !== Math.round(imgW / 2) ||
    ...
    const renderContentArea = (w, h) => (_jsxs("div", { style: {
            width: w,
            height: h,
            position: 'relative',
            overflow: 'hidden',
        }, children: [imageChanged && prevImg && crossfadeAlpha < 1 && (_jsx("div", { style: {
                    position: 'absolute',
                    width: w,
                    height: h,
                    overflow: 'hidden',
                    opacity: 1 - crossfadeAlpha,
                }, children: _jsx("img", { alt: "", src: prevImg, style: {
                        width: w,
                        height: h,
                        transformOrigin: '0 0',
                    ...
                    opacity: imageChanged ? crossfadeAlpha : 1,
                }, children: [_jsx("img", { alt: "", src: img, style: {
                            width: w,
                            height: h,
                            transformOrigin: '0 0',
                            transform: transformStyle,
                            ...
                            transform: transformStyle,
                            pointerEvents: 'none',
                        }, children: renderInsightOverlays() })] }), spinningPointer && (_jsx("img", { alt: "", src: mouseLoading, style: {
                    position: 'absolute',
                ...
                } })), showCursor && !spinningPointer && (_jsx("img", { alt: "", src: currentPointerImg, style: {
                    position: 'absolute',
