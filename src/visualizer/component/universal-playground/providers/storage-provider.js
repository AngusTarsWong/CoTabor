...
export class LocalStorageProvider {
    messagesKey;
    resultsKey;
    maxStorageItems = 50; // Limit stored items to prevent quota issues
    ...
    async handleQuotaExceeded() {
        try {
            ...
            const playgroundKeys = keys.filter((key) => key.includes('playground') ||
                key.includes('agent') ||
 ...
 */
export function createStorageProvider(type = StorageType.INDEXEDDB, namespace = 'playground') {
    switch (type) {
...
export function detectBestStorageType() {
