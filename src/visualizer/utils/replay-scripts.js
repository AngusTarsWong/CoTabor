'use client';
import { mousePointer } from '@/visualizer/utils';
...
// fit rect to camera
export const cameraStateForRect = (rect, imageWidth, imageHeight) => {
...
export const mergeTwoCameraState = (cameraState1, cameraState2) => {
...
const normalizeDump = (dump) => {
    if (!dump)
        return null;
    return Array.isArray(dump.executions)
        ? dump
        : {
            sdkVersion: '',
            groupName: 'Execution',
...
export const generateAnimationScripts = (execution, task, imageWidth, imageHeight, executionIndex = 0) => {
    ...
    const scripts = [];
    let insightCameraState = undefined;
    ...
    tasksIncluded.forEach((task, index) => {
                ...
                if (actions.length > 0) {
                    ...
                    if (action.param) {
                        ...
                        for (const key in action.param) {
                            ...
                            if (typeof action.param[key] === 'object' &&
                                'center' in (action.param[key] || {})) {
                                locateElements.push(action.param[key]);
                            }
                        }
                    }
                }
        ...
        if (task.status !== 'finished') {
            ...
            const errorSubTitle = errorMsg.indexOf('NOT_IMPLEMENTED_AS_DESIGNED') > 0
