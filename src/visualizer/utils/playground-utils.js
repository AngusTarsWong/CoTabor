import { StaticPage, StaticPageAgent } from '@/web/static';
import { isZodObjectSchema, unwrapZodType } from '../types';
// Get action name based on type
export const actionNameForType = (type) => {
    if (!type)
        return '';
...
export const staticAgentFromContext = (context) => {
...
export const isRunButtonEnabled = (runButtonEnabled, needsStructuredParams, params, actionSpace, selectedType, promptValue) => {
    if (!runButtonEnabled) {
        return false;
    }
    ...
    if (needsStructuredParams) {
        ...
        if (action?.paramSchema && isZodObjectSchema(action.paramSchema)) {
            ...
            return Object.keys(shape).every((key) => {
                ...
                return (isOptional || (value !== undefined && value !== '' && value !== null));
            });
        }
