...
// Type guards
export const isZodObjectSchema = (schema) => {
    return (typeof schema === 'object' &&
        schema !== null &&
        ('shape' in schema || schema.type === 'ZodObject'));
};
export const isLocateField = (field) => {
    ...
    if (typeof field === 'object' && field !== null) {
        ...
        const description = fieldWithRuntime.description ||
            fieldWithRuntime._def?.description ||
            '';
        if (typeof description === 'string') {
            const desc = description.toLowerCase();
            if (desc.includes('input field') ||
        ...
        if (fieldWithRuntime.typeName === 'ZodObject' ||
            fieldWithRuntime.type === 'ZodObject') {
...
export const unwrapZodType = (field) => {
    let actualField = field;
    let isOptional = false;
    let hasDefault = false;
    while (actualField._def?.typeName === VALIDATION_CONSTANTS.ZOD_TYPES.OPTIONAL ||
        ...
        if (actualField._def?.typeName === VALIDATION_CONSTANTS.ZOD_TYPES.OPTIONAL) {
            isOptional = true;
        }
...
export const extractDefaultValue = (field) => {
