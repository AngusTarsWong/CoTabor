...
export async function parseStructuredParams(action, params, options = {}) {
    if (!action?.paramSchema || !('shape' in action.paramSchema)) {
        return [params.prompt || '', options];
    }
    const schema = action.paramSchema;
    const keys = schema && 'shape' in schema
...
export function validateStructuredParams(value, action) {
    if (!value.params) {
...
export async function executeAction(activeAgent, actionType, actionSpace, value, options) {
    if (actionType !== 'aiAct' && options.deepThink !== undefined) {
        console.warn('[Playground] Received deepThink in non-aiAct action options. deepThink is expected to be used with aiAct during migration.', {
            actionType,
    ...
    if (action && typeof activeAgent.callActionInActionSpace === 'function') {
        ...
        if (actionType === 'aiAssert') {
            const { pass, thought } = (await activeAgent?.aiAssert?.(prompt || '', undefined, {
