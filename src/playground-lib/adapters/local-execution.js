...
export class LocalExecutionAdapter extends BasePlaygroundAdapter {
    agent;
    agentFactory; // Factory function for recreating agent
    dumpUpdateCallback;
    progressCallback;
    _id; // Unique identifier for this local adapter instance
    currentRequestId; // Track current request to prevent stale callbacks
    ...
    async executeAction(actionType, value, options) {
        ...
        if (!this.agent) {
            throw new Error('No agent available. Please provide either an agent instance or agentFactory.');
        }
    ...
    async recreateAgent() {
        if (!this.agentFactory) {
            throw new Error('Cannot recreate agent: factory function not provided. Please provide agentFactory in PlaygroundConfig to enable agent recreation.');
        }
