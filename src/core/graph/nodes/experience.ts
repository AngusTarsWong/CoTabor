...
export const experienceNode = async (state: AgentState): Promise<Partial<AgentState>> => {
  ...
  const trajectoryLog = recentHistory.map((step, index) => {
    ...
    const skillName = step.action?.skill_name ? ` skill=${step.action.skill_name}` : "";
...
请提取高价值经验（无价值则留空）。仅输出 JSON。`;
    ...
    const insights: any = { site_insights: [], tool_insights: [], task_wisdom: [] };
    ...
    (distillation.site_insights || []).forEach((item) => {
      if (item?.content?.trim()) {
        insights.site_insights.push({ domain: item.domain || currentDomain, content: item.content.trim() });
      }
    });
    (distillation.tool_insights || []).forEach((item) => {
      if (item?.skillName?.trim() && item?.content?.trim()) {
        insights.tool_insights.push({ skillName: item.skillName.trim(), content: item.content.trim() });
      }
    });
    (distillation.task_wisdom || []).forEach((item) => {
      if (item?.trim()) {
        insights.task_wisdom.push(item.trim());
      }
    });

    if (insights.site_insights.length > 0 || insights.tool_insights.length > 0 || insights.task_wisdom.length > 0) {
