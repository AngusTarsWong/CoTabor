...
import { defaultRunDirName, getMidsceneRunSubDir, } from '@/shared/common';
import { MIDSCENE_CACHE, MIDSCENE_DEBUG_MODE, globalConfigManager, } from '@/shared/env';
import { getRunningPkgInfo } from '@/shared/node';
import { assert, logMsg } from '@/shared/utils';
import { escapeScriptTag, ifInBrowser, ifInWorker, uuid, } from '@/shared/utils';
 ...
 */
export function processCacheConfig(cache, cacheId) {
 ...
 */
export function insertScriptBeforeClosingHtml(filePath, scriptContent) {
...
export function reportHTMLContent(dumpData, reportPath, appendReport, withTpl = true) {
    let tpl = '';
    if (withTpl) {
        tpl = getReportTpl();
        if (!tpl) {
...
export function writeDumpReport(fileName, dumpData, appendReport) {
    if (ifInBrowser || ifInWorker) {
        console.log('will not write report in browser');
        return null;
    }
    const reportPath = path.join(getMidsceneRunSubDir('report'), `${fileName}.html`);
...
export function getTmpFile(fileExtWithoutDot) {
    if (ifInBrowser || ifInWorker) {
        return null;
    }
...
export function overlapped(container, target) {
    // container and the target have some part overlapped
    return (container.left < target.left + target.width &&
...
export function replacerForPageObject(_key, value) {
...
export function stringifyDumpData(data, indents) {
...
function getGitInfoAsync() {
    ...
    gitInfoPromise = Promise.all([
        execFileAsync('git', ['config', '--get', 'remote.origin.url']).then(({ stdout }) => stdout.trim(), () => ''),
        execFileAsync('git', ['config', '--get', 'user.email']).then(({ stdout }) => stdout.trim(), () => ''),
    ]).then(([repoUrl, userEmail]) => ({ repoUrl, userEmail }));
    return gitInfoPromise;
}
let lastReportedRepoUrl = '';
export async function uploadTestInfoToServer({ testUrl, serverUrl, }) {
    if (!serverUrl)
        return;
