...
import { ExecutionDump, GroupedActionDump, } from '../types';
import { isAutoGLM, isUITars } from '@/core/ai-model/auto-glm/util';
import yaml from 'js-yaml';
import { ReportGenerator } from '@/core/report-generator';
import { getVersion, processCacheConfig, reportHTMLContent } from '@/core/utils';
import { ScriptPlayer, buildDetailedLocateParam, parseYamlScript, } from '../yaml/index';
import { existsSync } from 'node:fs';
import { resolve } from 'node:path';
import { MIDSCENE_REPLANNING_CYCLE_LIMIT, ModelConfigManager, globalConfigManager, globalModelConfigManager, } from '@/shared/env';
import { getDebug } from '@/shared/logger';
import { assert, ifInBrowser, uuid } from '@/shared/utils';
import { defineActionSleep } from '../device';
import { TaskCache } from './task-cache';
import { TaskExecutionError, TaskExecutor, locatePlanForLocate, withFileChooser, } from './tasks';
...
export class Agent {
     ...
     */
    hasWarnedNonVLModel = false;
    executionDumpIndexByRunner = new WeakMap();
    fullActionSpace;
    reportGenerator;
    ...
    constructor(interfaceInstance, opts) {
        ...
        this.opts = Object.assign({
            ...
            groupDescription: '',
        }, opts || {}, opts?.replanningCycleLimit === undefined &&
        ...
        if (opts?.modelConfig &&
            ...
            throw new Error(`opts.modelConfig must be a plain object map of env keys to values, but got ${typeof opts?.modelConfig}`);
        }
        ...
        if (cacheConfigObj) {
            this.taskCache = new TaskCache(cacheConfigObj.id, cacheConfigObj.enabled, undefined, // cacheFilePath
            {
    ...
    async setAIActContext(prompt) {
        if (this.aiActContext) {
            console.warn('aiActContext is already set, and it is called again, will override the previous setting');
        }
    ...
    wrapActionInActionSpace(name) {
        return async (param) => {
            return await this.callActionInActionSpace(name, param);
        };
    }
    async callActionInActionSpace(type, opt) {
        ...
        const plans = [actionPlan].filter(Boolean);
    ...
    async aiInput(locatePromptOrValue, locatePromptOrOpt, optOrUndefined) {
        ...
        if (typeof locatePromptOrOpt === 'object' &&
        ...
        assert(typeof value === 'string' || typeof value === 'number', 'input value must be a string or number, use empty string if you want to clear the input');
    ...
    async aiAct(taskPrompt, opt) {
        ...
        const runAiAct = async () => {
            ...
            const noIndividualLocateModel = modelConfigForPlanning.modelName ===
            ...
            if (deepLocate && includeBboxInPlanning) {
                console.warn('deepLocate option is ignored when includeBboxInPlanning is true (same model for planning and default intent without deepThink). Locate is already done during planning.');
            }
            ...
            const { output: actionOutput } = await this.taskExecutor.action(taskPrompt, modelConfigForPlanning, defaultIntentModelConfig, includeBboxInPlanning, this.aiActContext, cacheable, replanningCycleLimit, imagesIncludeCount, deepThink, fileChooserAccept, includeBboxInPlanning ? undefined : deepLocate, abortSignal);
            // update cache
    ...
    async describeElementAtPoint(center, opt) {
        ...
        while (!success && retryCount < retryLimit) {
            ...
            verifyResult = await this.verifyLocator(resultPrompt, undefined, center, opt);
    ...
    async verifyLocator(prompt, locateOpt, expectCenter, verifyLocateOption) {
        ...
        const pass = distance <= (verifyLocateOption?.centerDistanceThreshold || 20) ||
            included;
        const verifyResult = {
            pass,
            rect: verifyRect,
    ...
    async aiAssert(assertion, msg, opt) {
        ...
        const serviceOpt = {
            ...
            screenshotIncluded: opt?.screenshotIncluded ??
        ...
        try {
            ...
            if (error instanceof TaskExecutionError) {
                ...
                const rawMessage = errorTask?.errorMessage ||
    ...
    async evaluateJavaScript(script) {
        if (!this.interface.evaluateJavaScript) {
    ...
    processCacheConfig(opts) {
        ...
        if (opts.cache === true) {
            throw new Error('cache: true requires an explicit cache ID. Please provide:\n' +
        ...
        if (typeof cacheConfig === 'object' && cacheConfig !== null) {
            ...
            if (rawStrategy === undefined) {
                ...
                throw new Error(`cache.strategy must be a string when provided, but received type ${typeof rawStrategy}`);
            }
            if (!isValidCacheStrategy(strategyValue)) {
                throw new Error(`cache.strategy must be one of ${CACHE_STRATEGY_VALUES}, but received "${strategyValue}"`);
            }
...
export const createAgent = (interfaceInstance, opts) => {
