...
import { extractLastDumpScriptSync, getBaseUrlFixScript, streamImageScriptsToFile, } from './dump/html-utils';
...
export class ReportMergingTool {
    ...
    mergeReports(reportFileName = 'AUTO', opts) {
        ...
        const resolvedName = reportFileName === 'AUTO'
        ...
        if (reportFileName !== 'AUTO' && existsSync(outputFilePath)) {
            if (!overwrite) {
                throw new Error(`Report file already exists: ${outputFilePath}\nSet overwrite to true to overwrite this file.`);
            }
        ...
        try {
            ...
            for (let i = 0; i < this.reportInfos.length; i++) {
                ...
                if (this.isDirectoryModeReport(reportInfo.reportFilePath)) {
                    ...
                    const mergedScreenshotsDir = path.join(path.dirname(outputFilePath), 'screenshots');
