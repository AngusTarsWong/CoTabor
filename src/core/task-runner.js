...
import { ExecutionDump, } from '@/core/types';
...
export class TaskRunner {
    ...
    async getUiContext(options) {
        const now = Date.now();
        const shouldReuse = !options?.forceRefresh &&
            this.lastUiContext &&
    ...
    normalizeStatusFromError(options, errorMessage) {
        ...
        assert(options?.allowWhenError, errorMessage ||
    ...
    async flush(options) {
        if (this.status === 'init' && this.tasks.length > 0) {
            console.warn('illegal state for task runner, status is init but tasks are not empty');
        }
        ...
        while (taskIndex < this.tasks.length) {
            ...
            try {
                ...
                assert(['Insight', 'Action Space', 'Planning'].indexOf(task.type) >= 0, `unsupported task type: ${task.type}`);
                ...
                if (task.type === 'Insight') {
                    assert(task.subType === 'Query' ||
                        task.subType === 'Assert' ||
                        task.subType === 'WaitFor' ||
                        task.subType === 'Boolean' ||
                        task.subType === 'Number' ||
                        task.subType === 'String', `unsupported service subType: ${task.subType}`);
        ...
        if (!successfullyCompleted) {
            ...
            const messageBase = errorTask?.errorMessage ||
