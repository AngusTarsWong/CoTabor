 ...
 */
export function pointToBbox(x, y, bboxSize = defaultBboxSize) {
...
export function fillBboxParam(locate, width, height, modelFamily) {
...
export function adaptQwen2_5Bbox(bbox) {
    if (bbox.length < 2) {
...
export function adaptDoubaoBbox(bbox, width, height) {
...
export function adaptBbox(bbox, width, height, modelFamily) {
...
// x1, y1, x2, y2 -> 0-1000
export function normalized01000(bbox, width, height) {
    return [
...
// y1, x1, y2, x2 -> 0-1000
export function adaptGeminiBbox(bbox, width, height) {
...
export function adaptBboxToRect(bbox, width, height, offsetX = 0, offsetY = 0, rightLimit = width, bottomLimit = height, modelFamily, scale = 1) {
 ...
 */
export function expandSearchArea(rect, screenSize) {
...
export async function markupImageForLLM(screenshotBase64, tree, size) {
...
export function buildYamlFlowFromPlans(plans, actionSpace) {
    const flow = [];
...
export const RectSchema = PointSchema.and(SizeSchema).and(z.object({
...
export const ifMidsceneLocatorField = (field) => {
...
export const dumpMidsceneLocatorField = (field) => {
...
export const findAllMidsceneLocatorField = (zodType, requiredOnly) => {
    if (!zodType) {
        return [];
    }
...
export const dumpActionParam = (jsonObject, zodSchema) => {
        ...
        if (fieldValue) {
            ...
            if (typeof fieldValue === 'string') {
                ...
                if (fieldValue.prompt) {
                    ...
                    else if (typeof fieldValue.prompt === 'object' &&
 ...
 */
export const parseActionParam = (rawParam, zodSchema, options) => {
 ...
 */
export const getReadableTimeString = (format = 'YYYY-MM-DD HH:mm:ss', timestamp) => {
