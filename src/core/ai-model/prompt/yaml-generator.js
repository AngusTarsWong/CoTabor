 ...
 */
export const getScreenshotsForLLM = (events, maxScreenshots = 1) => {
    // Find events with screenshots, prioritizing navigation and click events
    const eventsWithScreenshots = events.filter((event) => event.screenshotBefore ||
    ...
    for (const event of sortedEvents) {
        ...
        const screenshot = event.screenshotWithBox ||
            event.screenshotAfter ||
 ...
 */
export const filterEventsByType = (events) => {
    return {
 ...
 */
export const createEventCounts = (filteredEvents, totalEvents) => {
    return {
 ...
 */
export const extractInputDescriptions = (inputEvents) => {
    return inputEvents
        .map((event) => ({
 ...
 */
export const processEventsForLLM = (events) => {
 ...
 */
export const prepareEventSummary = (events, options = {}) => {
 ...
 */
export const createMessageContent = (promptText, screenshots = [], includeScreenshots = true) => {
 ...
 */
export const generateYamlTest = async (events, options, modelConfig) => {
    try {
        // Validate input
 ...
 */
export const generateYamlTestStream = async (events, options, modelConfig) => {
    try {
        // Validate input
        validateEvents(events);
        // Prepare event summary using shared utilities
