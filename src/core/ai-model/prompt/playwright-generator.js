...
import { createEventCounts, createMessageContent, extractInputDescriptions, filterEventsByType, getScreenshotsForLLM, prepareEventSummary, processEventsForLLM, validateEvents, } from './yaml-generator';
 ...
 */
export const generatePlaywrightTest = async (events, options, modelConfig) => {
    // Validate input
    ...
    // Create prompt text
    const promptText = `Generate a Playwright test using @/web/playwright that reproduces this recorded browser session. The test should be based on the following events and follow the structure of the example provided. Make the test descriptive with appropriate assertions and validations.
...
4. Implement a test that uses Cotabor AI methods (aiTap, aiInput, aiAssert, etc.)
...
Important: Return ONLY the raw Playwright test code. Do NOT wrap the response in markdown code blocks (no \`\`\`typescript, \`\`\`javascript or \`\`\`). Start directly with the code content.`;
    ...
    const messageContent = createMessageContent(promptText, screenshots, options.includeScreenshots !== false);
    // Create system prompt
    const systemPrompt = `You are an expert test automation engineer specializing in Playwright and Midscene. 
Your task is to generate a complete, executable Playwright test using @/web/playwright that reproduces a recorded browser session.
 ...
 */
export const generatePlaywrightTestStream = async (events, options, modelConfig) => {
    // Validate input
    validateEvents(events);
    // Prepare event summary using shared utilities
    const summary = prepareEventSummary(events, {
