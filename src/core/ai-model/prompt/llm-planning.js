...
import { getZodDescription, getZodTypeName, } from '@/shared/zod-schema-utils';
...
const findDefaultValue = (field) => {
    ...
    while (current && !visited.has(current)) {
        ...
        if (!currentWithDef._def?.typeName)
            break;
        ...
        if (currentWithDef._def.typeName === 'ZodOptional' ||
...
export const descriptionForAction = (action, locatorSchemaTypeDescription, includeBbox = false) => {
    const tab = '  ';
    const fields = [];
        ...
        if (isZodObject && schema.shape) {
            ...
            for (const [key, field] of Object.entries(shape)) {
                if (field && typeof field === 'object') {
                    ...
                    const isOptional = typeof field.isOptional ===
                        'function' &&
                        field.isOptional();
...
export async function systemPromptToTaskPlanning({ actionSpace, modelFamily, includeBbox, includeThought, includeSubGoals, }) {
    const preferredLanguage = getPreferredLanguage();
    // Validate parameters: if includeBbox is true, modelFamily must be defined
    if (includeBbox && !modelFamily) {
        throw new Error('modelFamily cannot be undefined when includeBbox is true. A valid modelFamily is required for bbox-based location.');
    }
    const actionDescriptionList = actionSpace.map((action) => {
        return descriptionForAction(action, vlLocateParam(includeBbox ? modelFamily : undefined), includeBbox);
    });
