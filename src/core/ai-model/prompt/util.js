 ...
 */
export function extractXMLTag(xmlString, tagName) {
    ...
    if (lastCloseIndex === -1) {
        ...
        const content = nextTagIndex === -1 ? remaining : remaining.substring(0, nextTagIndex);
        return content.trim();
    }
 ...
 */
export function parseSubGoalsFromXML(xmlContent) {
    const subGoals = [];
 ...
 */
export function parseMarkFinishedIndexes(xmlContent) {
    const indexes = [];
