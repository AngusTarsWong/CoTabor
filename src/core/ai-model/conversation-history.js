export class ConversationHistory {
    ...
    mergeSubGoals(subGoals) {
        ...
        const existingByIndex = new Map(this.subGoals.map((goal) => [goal.index, goal]));
        const mergedSubGoals = subGoals.map((goal) => {
            ...
            return {
                ...goal,
                description: hasNonEmptyDescription || !existingGoal
                    ? goal.description
    ...
    subGoalsToText() {
        ...
        const currentGoal = this.subGoals.find((goal) => goal.status === 'running') ||
