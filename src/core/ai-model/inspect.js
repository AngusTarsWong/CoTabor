import { generateElementByPoint, generateElementByRect, } from '@/shared/extractor/dom-util';
import { cropByRect, paddingToMatchBlockByBase64, preProcessImageUrl, scaleImage, } from '@/shared/img';
...
import { extractDataQueryPrompt, parseXMLExtractionResponse, systemPromptToExtract, } from './prompt/extraction';
import { findElementPrompt, systemPromptToLocateElement, } from './prompt/llm-locator';
import { sectionLocatorInstruction, systemPromptToLocateSection, } from './prompt/llm-section-locator';
import { orderSensitiveJudgePrompt, systemPromptToJudgeOrderSensitive, } from './prompt/order-sensitive-judge';
import { AIResponseParseError, callAI, callAIWithObjectResponse, callAIWithStringResponse, } from './service-caller/index';
...
export async function AiLocateElement(options) {
    ...
    if (isAutoGLM(modelFamily)) {
        ...
        if (parsed.error || !parsed.coordinates) {
            ...
            const element = generateElementByPoint([finalX, finalY], targetElementDescriptionText);
    ...
    let res;
    try {
        res = await callAIWithObjectResponse(msgs, modelConfig, { abortSignal: options.abortSignal });
    }
    catch (callError) {
        ...
        const rawResponse = callError instanceof AIResponseParseError
            ? callError.rawResponse
            : errorMessage;
        const usage = callError instanceof AIResponseParseError ? callError.usage : undefined;
        return {
            rect: undefined,
            parseResult: {
                elements: [],
...
export async function AiLocateSection(options) {
    ...
    let result;
    try {
        result = await callAIWithObjectResponse(msgs, modelConfig, { abortSignal: options.abortSignal });
    }
    catch (callError) {
        // Return error with usage and rawResponse if available
        const errorMessage = callError instanceof Error ? callError.message : String(callError);
    ...
    if (sectionBbox) {
        ...
        const referenceRects = referenceBboxList
            ...
            .map((bbox) => {
            return adaptBboxToRect(bbox, context.shotSize.width, context.shotSize.height, 0, 0, context.shotSize.width, context.shotSize.height, modelFamily);
        });
    ...
    if (sectionRect) {
        ...
        const croppedResult = await cropByRect(screenshotBase64, sectionRect, modelFamily === 'qwen2.5-vl');
        const scaleRatio = 2;
...
export async function AiExtractElementInfo(options) {
...
export async function AiJudgeOrderSensitive(description, callAIFn, modelConfig) {
