...
export async function autoGLMPlanning(userInstruction, options) {
    const { conversationHistory, context, modelConfig, actionContext } = options;
    const systemPrompt = getAutoGLMPlanPrompt(modelConfig.modelFamily) +
        (actionContext
    ...
    const { content: rawResponse, usage } = await callAIWithStringResponse(msgs, modelConfig, { abortSignal: options.abortSignal });
    ...
    try {
        ...
        throw new AIResponseParseError(`Parse error: ${errorMessage}`, JSON.stringify(rawResponse, undefined, 2), usage);
    }
    conversationHistory.append({
        role: 'assistant',
