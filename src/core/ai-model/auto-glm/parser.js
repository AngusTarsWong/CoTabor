...
export const extractValueAfter = (src, key) => {
...
export function parseAction(response) {
    ...
    try {
        ...
        if (trimmedResponse.startsWith('do(action="Type"') ||
        ...
        if (trimmedResponse.startsWith('do(')) {
            ...
            if (!actionMatch)
                throw new Error(`Failed to extract action type from do() call; raw="${trimmedResponse}"`);
            const actionType = actionMatch[1];
            const baseAction = { _metadata: 'do', think: response.think };
            switch (actionType) {
                case 'Tap': {
                    const elementMatch = trimmedResponse.match(/element=\[(\d+),(\d+)\]/);
                    if (!elementMatch)
                        throw new Error(`Failed to extract element coordinates for Tap; raw="${trimmedResponse}"`);
                    return {
                        ...baseAction,
                        action: 'Tap',
                ...
                case 'Double Tap': {
                    const elementMatch = trimmedResponse.match(/element=\[(\d+),(\d+)\]/);
                    if (!elementMatch)
                        throw new Error(`Failed to extract element coordinates for Double Tap; raw="${trimmedResponse}"`);
                    return {
                        ...baseAction,
                        action: 'Double Tap',
                        element: [Number(elementMatch[1]), Number(elementMatch[2])],
                ...
                case 'Swipe': {
                    ...
                    if (!startMatch || !endMatch)
                        throw new Error(`Failed to extract start/end coordinates for Swipe; raw="${trimmedResponse}"`);
                    return {
                        ...baseAction,
                        action: 'Swipe',
                ...
                case 'Long Press': {
                    const elementMatch = trimmedResponse.match(/element=\[(\d+),(\d+)\]/);
                    if (!elementMatch)
                        throw new Error(`Failed to extract element coordinates for Long Press; raw="${trimmedResponse}"`);
                    return {
                        ...baseAction,
                        action: 'Long Press',
                        element: [Number(elementMatch[1]), Number(elementMatch[2])],
                ...
                case 'Launch': {
                    ...
                    return { ...baseAction, action: 'Launch', app };
                }
                case 'Back': {
                ...
                case 'Wait': {
                    ...
                    if (!durationMatch) {
                        throw new Error(`Failed to extract duration for Wait; raw="${trimmedResponse}"`);
                    }
...
export function parseAutoGLMResponse(content) {
...
export function parseAutoGLMLocateResponse(rawResponse) {
