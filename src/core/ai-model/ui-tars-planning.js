...
import { AIResponseParseError, callAIWithStringResponse, } from './service-caller/index';
...
export async function uiTarsPlanning(userInstruction, options) {
    ...
    ], modelConfig, { abortSignal: options.abortSignal });
    let convertedText;
    let parsed;
    try {
        ...
        throw new AIResponseParseError(`Parse error: ${errorMessage}`, JSON.stringify(res.content, undefined, 2), res.usage);
    }
    const { shotSize } = context;
    ...
    parsed.forEach((action) => {
        ...
        if (actionType === 'click') {
            ...
            const locate = {
                ...
                bbox: pointToBbox({ x: point[0], y: point[1] }, shotSize.width, shotSize.height),
            };
            ...
            transformActions.push({
                ...
                param: {
                    from: {
                        prompt: action.thought || '',
                        bbox: pointToBbox({ x: startPoint[0], y: startPoint[1] }, shotSize.width, shotSize.height),
                    },
                    to: {
                        prompt: action.thought || '',
                        bbox: pointToBbox({ x: endPoint[0], y: endPoint[1] }, shotSize.width, shotSize.height),
                    },
                },
                thought: action.thought || '',
            });
    ...
    if (transformActions.length === 0) {
        ...
        if (parsed.length === 0) {
            ...
            if (res.content.includes('Thought:') &&
                !res.content.includes('Action:')) {
                errorDetails.push('Response contains "Thought:" but missing "Action:" line');
            }
            else {
        ...
        // Throw AIResponseParseError with usage and rawResponse preserved
        throw new AIResponseParseError(errorMessage, JSON.stringify(res.content, undefined, 2), res.usage);
    }
