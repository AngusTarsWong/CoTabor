...
import { MIDSCENE_LANGFUSE_DEBUG, MIDSCENE_LANGSMITH_DEBUG, MIDSCENE_MODEL_MAX_TOKENS, OPENAI_MAX_TOKENS, globalConfigManager, } from '@/shared/env';
...
async function createChatClient({ modelConfig, }) {
    ...
    if (httpProxy) {
        ...
        if (ifInBrowser) {
            warnProxy('HTTP proxy is configured but not supported in browser environment');
        }
        else {
        ...
        if (ifInBrowser) {
            warnProxy('SOCKS proxy is configured but not supported in browser environment');
        }
        else {
            try {
                // Dynamic import with variable to avoid bundler static analysis
                ...
                const socksType = protocol === 'socks4' ? 4 : protocol === 'socks5' ? 5 : 5;
...
export async function callAI(messages, modelConfig, options) {
    const { completion, modelName, modelDescription, uiTarsModelVersion, modelFamily, } = await createChatClient({
        modelConfig,
    });
    const maxTokens = globalConfigManager.getEnvConfigValueAsNumber(MIDSCENE_MODEL_MAX_TOKENS) ??
    ...
    const mergedEnableReasoning = (() => {
        const normalizedDeepThink = options?.deepThink === 'unset' ? undefined : options?.deepThink;
        if (normalizedDeepThink === true)
            return true;
        if (normalizedDeepThink === false)
            return false;
        ...
        if (isStreaming) {
            ...
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                try {
                    ...
                    if (!result.choices) {
                        throw new Error(`invalid response from LLM service: ${JSON.stringify(result)}`);
                    }
...
export async function callAIWithObjectResponse(messages, modelConfig, options) {
    ...
    if (typeof jsonContent !== 'object') {
        throw new AIResponseParseError(`failed to parse json response from model (${modelConfig.modelName}): ${response.content}`, response.content, response.usage);
    }
    return {
        content: jsonContent,
...
export async function callAIWithStringResponse(msgs, modelConfig, options) {
    const { content, usage } = await callAI(msgs, modelConfig, {
        abortSignal: options?.abortSignal,
    });
    return { content, usage };
}
export function extractJSONFromCodeBlock(response) {
    try {
...
export function preprocessDoubaoBboxJson(input) {
...
export function resolveReasoningConfig({ reasoningEnabled, reasoningEffort, reasoningBudget, modelFamily, }) {
...
export function safeParseJson(input, modelFamily) {
