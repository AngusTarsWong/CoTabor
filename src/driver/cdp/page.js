...
export class ChromeExtensionProxyPage {
    ...
    constructor() {
        ...
        const cdpClient = {
            send: (command, params) => this.sendCommandToDebugger(command, params),
        };
    ...
    async setActiveTabId(tabId) {
        if (this.activeTabId) {
            throw new Error(`Active tab id is already set to ${this.activeTabId}, cannot change to ${tabId}`);
        }
        // 切换到该标签页
    ...
    async ensureDebuggerAttached() {
        ...
        if (tab.url?.startsWith('chrome://')) {
            throw new Error('Cannot attach debugger to chrome:// pages. Please use a normal page (http/https/file).');
        }
        try {
            ...
            if (this._continueWhenFailedToAttachDebugger) {
                console.warn('Failed to attach debugger, but continuing due to flag', error);
                return;
            }
            throw error;
        }
