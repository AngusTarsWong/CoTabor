...
export class ExperienceJobScheduler {
  async schedule(input: TaskMemoryCommitInput): Promise<TaskMemoryCommitResult> {
    ...
    const taskRun: TaskRunRecord = {
      ...
      runScope: input.finalState.dag_run_id ? "DAG_ROOT" : "ROOT",
      dagRunId: input.finalState.dag_run_id,
      subtaskDag: input.finalState.subtask_dag,
      schedulerRuntime: input.finalState.scheduler_runtime,
      dagExecutionMode: input.finalState.dag_execution_mode,
