...
function getLogStream(topic) {
    ...
    if (!logStreams.has(topicFileName)) {
        const logFile = path.join(getMidsceneRunSubDir('log'), `${topicFileName}.log`);
...
export function getDebug(topic, options) {
...
export function enableDebug(topic) {
    if (ifInNode) {
...
export function cleanupLogStreams() {
    if (!ifInNode)
        return;
