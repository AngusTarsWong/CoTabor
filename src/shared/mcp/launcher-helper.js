 ...
 * @example
 * ```typescript
 * import { createMCPServerLauncher } from '@/shared/mcp';
 ...
 * @internal
 */
export function createMCPServerLauncher(config) {
    ...
    function validateAgent() {
        ...
        if (!device) {
            throw new Error(`Agent must have an 'interface' property that references the underlying device.
...
Solution: Check that your agent constructor properly sets the interface property.`);
    ...
    return {
        ...
        async launchHttp(options) {
            ...
            if (verbose) {
                console.log(`${platformName} MCP Server started on http://${result.host}:${result.port}/mcp`);
            }
            return result;
        },
    };
