...
/**
 * Rslib plugin to inject report HTML from @/core dist into MCP bundle.
 * This runs after build and reads the already-injected HTML from core.
 *
 * Prerequisites:
 * - @/report must be in devDependencies to ensure correct build order
 * - @/core dist must exist with injected HTML
 *
 * @param packageDir - The directory of the MCP package (use __dirname)
 */
export function injectReportHtmlFromCore(packageDir) {
    return {
        name: 'inject-report-html-from-core',
        setup(api) {
            api.onAfterBuild(() => {
                const coreUtilsPath = path.resolve(packageDir, '..', 'core', 'dist', 'lib', 'utils.js');
                if (!fs.existsSync(coreUtilsPath)) {
                    console.warn('[inject-report-html] @/core dist not found, skipping');
                    return;
                }
                const coreContent = fs.readFileSync(coreUtilsPath, 'utf-8');
                if (!coreContent.includes(REPLACED_MARK)) {
                    console.warn('[inject-report-html] HTML not found in core dist. Ensure report builds first.');
                    return;
                }
                ...
                for (const file of jsFiles) {
                    ...
                    if (content.includes(REPLACED_MARK)) {
                        if (REG_EXP_FOR_REPLACE.test(content)) {
                            fs.writeFileSync(filePath, content.replace(REG_EXP_FOR_REPLACE, () => finalContent));
                            console.log(`[inject-report-html] Updated: ${file}`);
                            injectedCount++;
                        }
                    }
                    else if (content.includes(`'${MAGIC_STRING}'`)) {
                        fs.writeFileSync(filePath, content.replace(`'${MAGIC_STRING}'`, () => finalContent));
                        console.log(`[inject-report-html] Injected: ${file}`);
                        injectedCount++;
                    }
                }
                if (injectedCount > 0) {
                    console.log(`[inject-report-html] Completed: ${injectedCount} file(s)`);
                }
            });
        },
    };
