...
import { setIsMcp } from '@/shared/utils';
 ...
 */
export function launchMCPServer(server, args) {
    if (args.mode === 'http') {
    ...
    async launchHttp(options) {
        ...
        const server = app
            ...
            if (error.code === 'EADDRINUSE') {
                ...
                console.error(`ERROR: Permission denied to bind to port ${options.port}.\nPorts below 1024 require root/admin privileges.\nPlease use a port above 1024 or run with elevated privileges.`);
            }
            else {
                console.error(`ERROR: Failed to start HTTP server on ${host}:${options.port}\n` +
