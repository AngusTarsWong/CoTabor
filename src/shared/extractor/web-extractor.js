import { CONTAINER_MINI_HEIGHT, CONTAINER_MINI_WIDTH, NodeType, } from '../constants/index';
import { isAElement, isButtonElement, isContainerElement, isFormElement, isImgElement, isTextElement, } from './dom-util';
import { descriptionOfTree } from './tree';
import { elementRect, getNodeAttributes, getPseudoElementContent, getRect, getTopDocument, logger, midsceneGenerateHash, setDebugMode, } from './util';
let indexId = 0;
...
export function collectElementInfo(node, currentWindow, currentDocument, baseZoom = 1, basePoint = { left: 0, top: 0 }, isContainer = false) {
    const rect = elementRect(node, currentWindow, currentDocument, baseZoom);
    if (!rect) {
        return null;
    }
    if (rect.width < CONTAINER_MINI_WIDTH ||
...
// @deprecated
export function extractTextWithPosition(initNode, debugMode = false) {
...
export function extractTreeNodeAsString(initNode, visibleOnly = false, debugMode = false) {
    const elementNode = extractTreeNode(initNode, debugMode);
    return descriptionOfTree(elementNode, undefined, false, visibleOnly);
}
export function extractTreeNode(initNode, debugMode = false) {
...
export function mergeElementAndChildrenRects(node, currentWindow, currentDocument, baseZoom = 1) {
