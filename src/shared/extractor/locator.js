...
import { getNodeFromCacheList, getRect, isElementPartiallyInViewport, logger, } from './util';
...
const buildCurrentElementXpath = (element, isOrderSensitive, isLeafElement, limitToCurrentDocument = false) => {
    ...
    const parentPath = element.parentNode
        ? getElementXpath(element.parentNode, isOrderSensitive, false, limitToCurrentDocument)
        : '';
...
export const getElementXpath = (element, isOrderSensitive = false, isLeafElement = false, limitToCurrentDocument = false) => {
    // process text node
    ...
    if (isSvgElement(el)) {
        ...
        while (parent && parent.nodeType === Node.ELEMENT_NODE) {
            ...
            if (!isSvgElement(parentEl)) {
                return getElementXpath(parentEl, isOrderSensitive, isLeafElement, limitToCurrentDocument);
            }
        ...
        if (fallbackParent && fallbackParent.nodeType === Node.ELEMENT_NODE) {
            return getElementXpath(fallbackParent, isOrderSensitive, isLeafElement, limitToCurrentDocument);
        }
        return '';
    }
    return buildCurrentElementXpath(el, isOrderSensitive, isLeafElement, limitToCurrentDocument);
...
export function getXpathsById(id) {
...
export function getXpathsByPoint(point, isOrderSensitive) {
    let currentWindow = typeof window !== 'undefined' ? window : undefined;
    let currentDocument = typeof document !== 'undefined' ? document : undefined;
    let { left, top } = point;
    let depth = 0;
    const MAX_DEPTH = 10;
    let xpathPrefix = '';
        ...
        if (tag === 'iframe' || tag === 'frame') {
            try {
                ...
                if (contentWindow && contentDocument) {
                    const localPoint = translatePointToIframeCoordinates({ left, top }, element, currentWindow);
...
export function getNodeInfoByXpath(xpath) {
    const parts = xpath
...
export function getElementInfoByXpath(xpath) {
    const node = getNodeInfoByXpath(xpath);
    if (!node)
        return null;
    let targetWindow = typeof window !== 'undefined' ? window : undefined;
    let targetDocument = typeof document !== 'undefined' ? document : undefined;
