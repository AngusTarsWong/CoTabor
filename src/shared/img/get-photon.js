...
export default async function getPhoton() {
    ...
    try {
        ...
        if (!photonModule.PhotonImage.new_from_byteslice &&
            !photonModule.PhotonImage.new_from_base64) {
            throw new Error('PhotonImage.new_from_byteslice or new_from_base64 is not available');
        }
        isInitialized = true;
        return photonModule;
    }
        ...
        if (ifInBrowser) {
            ...
            try {
                const { createCanvasFallbackModule } = await import('./canvas-fallback');
 ...
 */
export function isUsingCanvasFallback() {
