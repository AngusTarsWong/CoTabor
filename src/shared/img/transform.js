 ...
 */
export async function resizeAndConvertImgBuffer(inputFormat, inputData, newSize) {
    if (typeof inputData === 'string')
        throw Error('inputData is base64, use resizeImgBase64 instead');
    assert(newSize && newSize.width > 0 && newSize.height > 0, 'newSize must be positive');
    ...
    if (ifInNode) {
        ...
        try {
            ...
            if (newSize.width === originalWidth &&
    ...
    const inputImage = bytesliceResult instanceof Promise
...
export const createImgBase64ByFormat = (format, body) => {
    return `data:image/${format};base64,${body}`;
};
export async function resizeImgBase64(inputBase64, newSize) {
 ...
 */
export function zoomForGPT4o(originalWidth, originalHeight) {
...
export async function photonFromBase64(base64) {
...
export async function paddingToMatchBlock(image, blockSize = 28) {
...
export async function paddingToMatchBlockByBase64(imageBase64, blockSize = 28) {
...
export async function cropByRect(imageBase64, rect, paddingImage) {
...
export async function photonToBase64(image, quality = 90) {
...
export const httpImg2Base64 = async (url) => {
    ...
    assert(contentType.startsWith('image/'), `The url ${url} is not a image, because of content-type in header is ${contentType}.`);
 ...
 */
export const localImg2Base64 = (imgPath, withoutHeader = false) => {
 ...
 */
export const preProcessImageUrl = async (url, convertHttpImage2Base64) => {
    if (typeof url !== 'string') {
        throw new Error(`url must be a string, but got ${url} with type ${typeof url}`);
    }
 ...
 */
export const parseBase64 = (fullBase64String) => {
    try {
 ...
 */
export async function scaleImage(imageBase64, scale) {
    if (scale <= 0) {
