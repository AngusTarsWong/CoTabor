...
const NotionTab: React.FC = () => {
  ...
  const handleManualBind = async () => {
    ...
    const rawIds = [manualL1.trim(), manualL2.trim(), manualL3.trim()];
    if (rawIds.some(id => !id)) { setManualError('请填写全部三个数据库 ID 或链接'); return; }
    ...
    try {
      ...
      const [l1Id, l2Id, l3Id] = rawIds.map(extractId);
      for (const [i, dbId] of [l1Id, l2Id, l3Id].entries()) {
        ...
        if (!res.ok) {
          ...
          throw new Error(`${ ['L1', 'L2', 'L3'][i] } 数据库验证失败: ${(err as any).message ?? res.statusText}`);
        }
      }
    ...
    <div style={card}>
      ...
      <div style={{ ...sectionBox, opacity: isLoggedIn || apiKey ? 1 : 0.5 }}>
        ...
        <p style={{ fontSize: '13px', color: '#6b7280', marginBottom: '12px', lineHeight: '1.5' }}>
          若您已在 Notion 中手动创建了 CoTabor 所需的三个数据库，可直接粘贴各数据库链接或 32 位 ID，跳过自动初始化。
        </p>
        {([
          { label: 'L1 肌肉记忆（DOM 操作规则）', value: manualL1, set: setManualL1 },
          { label: 'L2 技能记忆（API 参数规则）', value: manualL2, set: setManualL2 },
          { label: 'L3 战术记忆（SOP 步骤）',     value: manualL3, set: setManualL3 },
        ] as const).map(({ label, value, set }) => (
          <div key={label} style={{ marginBottom: '10px' }}>
            <label style={{ display: 'block', marginBottom: '4px', fontWeight: 500, fontSize: '13px', color: '#374151' }}>{label}</label>
            <input
              type="text"
              value={value}
              onChange={e => (set as React.Dispatch<React.SetStateAction<string>>)(e.target.value)}
              placeholder="https://notion.so/... 或 32位ID"
              style={inputStyle}
        ...
        <button
          ...
          style={{
            ...btn('#0f766e', manualStatus === 'loading' || (!isLoggedIn && !apiKey)),
            width: '100%', padding: '10px', fontSize: '14px', borderRadius: '6px',
          }}
        >
          {manualStatus === 'loading' ? '⏳ 验证中...'
           : manualStatus === 'success' ? '✅ 已验证并保存'
           : '🔗 验证并绑定数据库'}
        </button>
