...
*/
import { limitOpenNewTabScript } from '@/web/web-element';
...
export default class ChromeExtensionProxyPage {
    interfaceType = 'chrome-extension-proxy';
    forceSameTabNavigation;
    viewportSize;
    activeTabId = null;
    destroyed = false;
    isMobileEmulation = null;
    _continueWhenFailedToAttachDebugger = false;
    ...
    async ensureDebuggerAttached() {
        ...
        if (url.startsWith('chrome://')) {
            throw new Error('Cannot attach debugger to chrome:// pages, please use Midscene in a normal page with http://, https:// or file://');
        }
        ...
        try {
            ...
            if (this._continueWhenFailedToAttachDebugger) {
                console.warn('Failed to attach debugger, but continuing due to _continueWhenFailedToAttachDebugger flag', error);
                return;
            }
            throw error;
        }
    ...
    async sendCommandToDebugger(command, params, retryCount = 0) {
        ...
        try {
            ...
            const result = (await chrome.debugger.sendCommand({ tabId }, command, params));
            ...
            const errorMsg = error?.message || '';
            const isDetachError = errorMsg.includes('Debugger is not attached') ||
