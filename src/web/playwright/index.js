...
import { forceChromeSelectRendering as applyChromeSelectRendering, forceClosePopup, } from '../puppeteer/base-page';
...
export class PlaywrightAgent extends PageAgent {
    constructor(page, opts) {
        if (!page) {
            throw new Error('[midscene] PlaywrightAgent requires a valid Playwright page instance. Please make sure to pass a valid page object.');
        }
        ...
        if (forceChromeSelectRendering) {
            ...
            if (playwrightVersion && !semver.gte(playwrightVersion, '1.52.0')) {
                console.warn(`[midscene:error] forceChromeSelectRendering requires Playwright >= 1.52.0, but current version is ${playwrightVersion}. This feature may not work correctly.`);
            }
