...
import { escapeScriptTag, replaceIllegalPathCharsAndSpace, } from '@/shared/utils';
class MidsceneReporter {
    mergedFilename;
    testTitleToFilename = new Map();
    mode;
    outputFormat;
    ...
    static getMode(reporterType) {
        ...
        if (reporterType !== 'merged' && reporterType !== 'separate') {
            throw new Error(`Unknown reporter type in playwright config: ${reporterType}, only support 'merged' or 'separate'`);
        }
        return reporterType;
    }
    ...
    getReportFilename(testTitle) {
        if (this.mode === 'merged') {
            ...
            if (!testTitle)
                throw new Error('testTitle is required in separate mode');
    ...
    async updateReport(testData) {
        ...
        this.writeQueue = this.writeQueue.then(async () => {
            ...
            if (!tpl) {
                throw new Error('Report template not found. Ensure @/core is built correctly.');
            }
    ...
    async onEnd() {
        ...
        else if (this.outputFormat === 'html-and-external-assets' &&
            this.mode === 'separate') {
